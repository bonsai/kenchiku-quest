...
async def calc_beam(data: dict):
    """梁計算API: {b, h, P, L, E} -> {Z, I, sigma, delta, ratio, result}"""
...
async def calc_cpm(data: dict):
    """CPM批判パス計算API: {tasks} -> {critical_path, total_duration}"""
...
async def get_ontology(name: str):
    """オントロジー取得API: structure, environment, construction, regulation, common"""
    data = load_ontology(name)
    return data if data else {"error": f"ontology '{name}' not found"}

@app.post("/api/game/save")
async def game_save(player: dict):
    """セーブAPI（実際にはDBに保存。デモではファイル or メモリ）"""
    # TODO: DB or file persist
    return {"saved": True, "player": player.get("name")}

@app.post("/api/game/quiz")
async def game_quiz(data: dict):
    """クイズ判定API: {subject, question_id, answer} -> {correct, explanation, score}"""
    # TODO: load exam-questions json
